...
class DirectionPublisher(Node):
    def __init__(self):
        ...
        self.model = YOLO("best_new_new.pt")
    ...
    def control_turtlebot(self, contour, relative_position, distance):
        ...
        print("THis is the allignment indx",relative_position[0])
        ...
        first_start_point = (int(width / 3)+50, 0)
        first_end_point = (int(width / 3)+50, height)
        start_point = (int(2 * width / 3)+50, 0)
        end_point = (int(2 * width / 3)+50, height)

        # Draw the line
        color = (0, 255, 0)  # Green color in BGR
        thickness = 2        # Line thickness
        cv2.line(self.frame, first_start_point, first_end_point, color, thickness)
        cv2.line(self.frame, start_point, end_point, color, thickness)
        # Set angular velocity based on relative position (left/right)
        if 300 < distance <= 400:
            if relative_position[0] < -195:  # Move left
                self.align_vel_msg.angular.z = 1.0
                self.get_logger().info(f"{self.align_vel_msg}")
                self.align_publisher.publish(self.align_vel_msg)
                #self.arrow_detected=False
            elif relative_position[0] > 195:  # Move right
                self.align_vel_msg.angular.z = -1.0
                self.get_logger().info(f"{self.align_vel_msg}")
                self.align_publisher.publish(self.align_vel_msg)
                #self.arrow_detected=False
            ...
            if relative_position[0] < -130:  # Move left
                self.align_vel_msg.angular.z = 1.0
                self.get_logger().info(f"{self.align_vel_msg}")
                self.align_publisher.publish(self.align_vel_msg)
                #self.arrow_detected=False
            elif relative_position[0] > 130:  # Move right
                self.align_vel_msg.angular.z = -1.0
                self.get_logger().info(f"{self.align_vel_msg}")
                self.align_publisher.publish(self.align_vel_msg)
                #self.arrow_detected=False
            ...
            if relative_position[0] < -80:  # Move left
                self.align_vel_msg.angular.z = 1.0
                self.get_logger().info(f"{self.align_vel_msg}")
                self.align_publisher.publish(self.align_vel_msg)
                #self.arrow_detected=False
            elif relative_position[0] > 80:  # Move right
                self.align_vel_msg.angular.z = -1.0
                self.get_logger().info(f"{self.align_vel_msg}")
                self.align_publisher.publish(self.align_vel_msg)
                #self.arrow_detected=False
    ...
    # Rest of the processing logic from send_cmd_vel
        ...
        for arrow in detected_arrows:
            ...
            distance = self.arrow_distance_estimation(w, camMatrix)
