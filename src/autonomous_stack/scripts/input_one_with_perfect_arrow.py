...
class DirectionPublisher(Node):
    def __init__(self):
        ...
        self.border_size = 20
        self.video_port="/dev/video2"
        self.distance_ll = 50
        self.distance_ul = 100
        self.allignment_ll = 100 
        self.allignment_ul = 300
        self.relative_position_limit = self.height // 0.125
    ...
    def arrow_distance_estimation(self, w, camMatrix):
        ...
        distance = (((self.REAL_ARROW_WIDTH * focal_length) / object_width_in_pixels  )*self.scaling_factor)/2# Depth in cm
        #print(depth)
        # if distance>100:
        #     distance*1.2
        ...
        distance=0
        if(self.height==720):
                    distance = (((self.REAL_ARROW_WIDTH * focal_length) / object_width_in_pixels  )*(1000/1280)/25)# Depth in cm
                    #print(depth)
                    if distance>100:
                    ...
                    distance = (((self.REAL_ARROW_WIDTH * focal_length) / object_width_in_pixels  )*(100/1080))/2# Depth in cm
                    #print(depth)
                    if distance <50:
    ...
    def control_turtlebot(self, contour, relative_position, distance):
        ...
        cv2.line(self.frame, (self.height//3, 0), (self.height//3, self.width), (255,0,0), 2)
        cv2.line(self.frame, (2*(self.height//3), 0), (2*(self.height//3), self.width), (255,0,0), 2)
        #self.arrow_detected=False
        # Set angular velocity based on relative position (left/right)
        if self.allignment_ll < distance < self.allignment_ul:
            if relative_position[0] < -self.relative_position_limit:  # Move left
                self.align_vel_msg.angular.z = 1.0
                self.get_logger().info(f"coordinates comming up for alignment")
                self.get_logger().info(f"{relative_position[0]}")
                self.align_publisher.publish(self.align_vel_msg)
                #self.arrow_detected=False
            elif relative_position[0] > self.relative_position_limit:  # Move right
                self.align_vel_msg.angular.z = -1.0
                self.get_logger().info(f"coordinates comming up for alignment")
                self.get_logger().info(f"{relative_position[0]}")
                self.align_publisher.publish(self.align_vel_msg)
    ...
    def send_cmd_vel(self, ret,frame):
        with open('/home/pradheep/integrated_autonomous_stack/src/autonomous_stack/scripts/cameraMatrix.pkl', 'rb') as f:
            camMatrix = pickle.load(f)
        with open('/home/pradheep/integrated_autonomous_stack/src/autonomous_stack/scripts/dist.pkl', 'rb') as f:
        ...
        for contour in contours:
            ...
            if len(approx) == 7:
                ...
                if (
                    ...
                    340<=(base_angle1+base_angle2+head_angle1+head_angle2)<=400
                ):
