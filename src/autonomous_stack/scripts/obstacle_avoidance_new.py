...
class ObstacleAvoider(Node):
    ...
    def scan_callback(self, msg):
        ...
        if msg.data in ['Front_Left', 'Front_Right']:
            self.front_obstacle = True
            self.published_direction = msg.data
            self.avoidance_stage = 'ANGULAR'
            self.get_logger().info(f"Obstacle detected: {msg.data}")
            self.waiting_for_next_stage = True
    ...
    def avoid_obstacle(self):
        ...
        if self.avoidance_stage == 'ANGULAR':
            ...
            if self.front_obstacle:
                ...
                self.cmd_vel_msg.linear.x = 0.0
                # Rotate based on the last detected obstacle direction
                # self.cmd_vel_msg.angular.z = 0.5 if 'Front_Right' in str(self.last_obstacle_direction) else -0.5
                if self.initial_yaw is None:
                    self.initial_yaw = self.current_yaw
                if self.published_direction == "Front_Right" and self.last_obstacle_direction=="Front_Left":
                    self.cmd_vel_msg.angular.z = -0.5
                    self.rotating_angle = -0.5
                elif self.published_direction=="Front_Left" and self.last_obstacle_direction=="Front_Right":
                    self.cmd_vel_msg.angular.z=0.5
                    self.rotating_angle = 0.5
                elif self.published_direction == 'Front_Right':
                    self.cmd_vel_msg.angular.z = 0.5
                    self.rotating_angle = 0.5
                    self.last_obstacle_direction="Front_Right"
                elif self.published_direction == 'Front_Left':
                    self.cmd_vel_msg.angular.z = -0.5
                    self.rotating_angle = -0.5
                    self.last_obstacle_direction="Front_Left"
                self.cmd_vel_publisher.publish(self.cmd_vel_msg)
        ...
        # Linear motion stage 
            ...
            status_msg.data = 'enable'
            self.obstacle_avoidance_status_publisher.publish(status_msg)
            if current_time - self.linear_motion_start_time >= 3.0:
        ...
        # Yaw correction stage
            ...
            status_msg.data = 'enable'
            self.obstacle_avoidance_status_publisher.publish(status_msg)
            if abs(self.current_yaw - self.initial_yaw) < 5.0:
                self.get_logger().info(f"Correcting yaw: {self.current_yaw}")
                self.cmd_vel_msg.linear.x = 0.0
                self.cmd_vel_msg.angular.z = -self.rotating_angle
                self.cmd_vel_publisher.publish(self.cmd_vel_msg)
            else:
                self.get_logger().info("Yaw corrected, resetting state")
                self.cmd_vel_msg.linear.x = 1.0
